...
<script type="text/javascript">
    ...
    function createEditableDiv(propertyUuid, property, text) {
        ...
        jQuery.ajax({
            ...
            success:function (data, status, jqXHR) {
                ...
                jQuery.each(data, function (index, element) {
                    ...
                    var propertyDiv = createEditableDiv(element.propertyUuid, "property", element.property);
                    property.append(propertyDiv);

                    var value = createElement("td");
                    var valueDiv = createEditableDiv(element.propertyUuid, "propertyValue", element.propertyValue);
                    value.append(valueDiv);

                    var description = createElement("td");
                    var descriptionDiv = createEditableDiv(element.propertyUuid, "propertyDescription", element.propertyDescription);
                    description.append(descriptionDiv);

                    var remove = createElement("td");
                    var removeAnchor = createElement("a");
                    removeAnchor.attr({
                    ...
                    removeAnchor.bind({
                        click: function() {
                            jQuery.ajax({
                                url:"deleteProperty.form",
                                type:"POST",
                                data: {propertyUuid: element.propertyUuid},
                                complete:function () {
                    ...
                    removeAnchor.html("Remove Property");
                    remove.append(removeAnchor);
        ...
        $j("#definitionList").ajaxComplete(function () {
            $j('.edit').editable(
                    function (value, settings) {
                        ...
                        var propertyUuid = $j(this).attr("propertyUuid");
                        jQuery.ajax({
